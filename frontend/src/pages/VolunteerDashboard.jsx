...
export default function VolunteerDashboard() {
  ...
  useEffect(() => {
    const fetchReports = async () => {
      try {
        const url = zone
          ? `/reports?zone=${encodeURIComponent(zone)}`
          : "/reports";
  ...
  const resolveReport = async (id) => {
    try {
      ...
      setReports((p) =>
        p.map((r) => (r._id === id ? { ...r, status: "resolved" } : r)),
      );
    } catch (err) {
      alert(err.message);
    }
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <Helmet>
        ...
        <meta
          name="description"
          content="Manage animal welfare reports and contribute to saving animals across Tamil Nadu."
        />
      </Helmet>
              ...
              <div className="flex-wrap flex justify-between items-center">
                ...
                <div className="flex-1 mx-4">
                  <p className="text-sm font-medium text-slate-800">
                    {r.animal?.species?.toUpperCase() || "Unknown Species"}
                  </p>
                  ...
                  <p className="text-xs text-slate-400 mt-1 py-2 md:py-0">
                    Zone: {r.zone || "—"}
                  </p>
                </div>
                {r.reportedBy && (
